...
from rlkit.experimental.kuanfang.envs.drawer_pnp_push_decomp_commands import drawer_pnp_push_decomp_commands
...
ROOT_PATH = '/media/3tb/jason/pybullet_data/env6_td_pnp_push_all-obj/'

decomp_goals = True

for test_env_seed in args.test_env_seeds:
    if decomp_goals:
        data_save_path = os.path.join(ROOT_PATH, f"td_pnp_push_decomp-goals_seed{test_env_seed}.pkl")
        command = drawer_pnp_push_decomp_commands[test_env_seed]
    else:
        data_save_path = os.path.join(ROOT_PATH, f"td_pnp_push_goals_seed{test_env_seed}.pkl")
    ...
    env = roboverse.make('SawyerRigAffordances-v6', test_env=True, expl=True, **kwargs)
