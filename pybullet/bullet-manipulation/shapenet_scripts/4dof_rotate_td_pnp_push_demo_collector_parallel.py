...
def collect(id):
    state_env = roboverse.make('SawyerRigAffordancesNoSlide-v6', expl=True, reset_interval=args.reset_interval, **kwargs)
    ...
    for j in tqdm(range(args.num_trajectories_per_demo)):
        only_keep_successful = False
        final_reward = None

        while final_reward is None or (only_keep_successful and (final_reward != 0.0)):
            env.demo_reset()
            ...
            # the desired goal should be the last state of the trajectory
            for i in range(args.num_timesteps):
                trajectory['observations'][i]['desired_goal'] = trajectory['observations'][-1]['achieved_goal']
                trajectory['observations'][i]['state_desired_goal'] = trajectory['observations'][-1]['state_achieved_goal']
                trajectory['next_observations'][i]['desired_goal'] = trajectory['next_observations'][-1]['achieved_goal']
                trajectory['next_observations'][i]['state_desired_goal'] = trajectory['next_observations'][-1]['state_achieved_goal']
...
if __name__ == '__main__':
    ...
    prefix = f"/media/3tb/jason/pybullet_data/env6_td_pnp_push_limited/"
