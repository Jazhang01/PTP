...
class SawyerRigMultiobjTrayV0(SawyerBaseEnv):
    ...
    def __init__(self,
        ...
        self._fixed_tray_position = np.array([.7, -0.1, -.35])
        ...
        self._view_matrix_obs = bullet.get_view_matrix(
            target_pos=[.7, 0, -0.3], distance=0.35,
    ...
    def _load_table(self):
        ...
        if self._randomize:
            tray_pos = np.random.uniform(low=self._tray_position_low, high=self._tray_position_high)
        else:
            tray_pos = self._fixed_tray_position
        self.tray = bullet.objects.drawer_tray(pos=tray_pos, scale=0.165, rgba=self.sample_object_color())
    ...
    def get_demo_action(self, test=False):
        if test:
            action, done = self.move_obj(np.array([.65, -0.15, -.25]))
        else:
